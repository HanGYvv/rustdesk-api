...
## 搭配 [RustDesk Server](https://github.com/HanYvv/rustdesk-server) 使用更佳

> [RustDesk Server](https://github.com/HanYvv/rustdesk-server) fork 自 RustDesk Server 官方仓库，并合入 [lejianwen/rustdek-server](https://github.com/lejianwen/rustdesk-server) 以下功能：
>
> 1. 解决使用API链接超时问题
> 2. 可以强制登录后才能发起链接
> 3. 支持客户端websocket
...
- PC端API
  ...
  - 地址簿
  - 群组
  - 授权登录：支持 `github`、`google` 和 `OIDC` 登录，支持 `web 后台` 授权登录，支持 `LDAP`（AD 和 OpenLDAP 已测试），如果 API Server 配置了 LDAP
  - i18n
- Web Admin
  - 用户管理
  - 设备管理
  ...
  - i18n
  - 通过 web client 分享给游客
  - 服务器控制：一些官方的简单的指令 [WIKI](https://github.com/HanGYvv/rustdesk-api/wiki/Rustdesk-Command)
- Web Client
  - 自动获取API server
  - 自动获取ID服务器和KEY
  - 自动获取地址簿
...
![登录](docs/pc_login.png)

#### 地址簿和群组

| 地址簿 | 群组 |
| --- | --- |
| ![地址簿](docs/pc_ab.png) | ![群组](docs/pc_gr.png) |

### Web Admin

- 使用前后端分离，提供用户友好的管理界面，主要用来管理和展示。前端代码在 [rustdesk-api-web](https://github.com/HanGYvv/rustdesk-api-web)
- 后台访问地址是 `http://<your server>[:port]/_admin/`
- 初次安装管理员为用户名 `admin`，密码将在控制台打印，可以通过 [命令行](### CLI) 更改密码

![初始化管理员密码](./docs/init_admin_pwd.png)
  ...
  目前支持 `GitHub`、`Google` 以及 `OIDC`，需要创建一个 `OAuth App` 并配置到后台。对于 `Google` 和 `GitHub`，`Issuer` 和 `Scopes` 不需要填写；对于 `OIDC`，`Issuer` 是必须的，`Scopes` 是可选的，默认为 `openid,profile,email`，确保可以获取 `sub`、`email` 和 `preferred_username`。`GitHub OAuth App` 在 `Settings` -> `Developer settings` -> `OAuth Apps` -> `New OAuth App` 中创建，地址为 [开发者设置](https://github.com/settings/developers)。`Authorization callback URL` 填写 `http://<your server[:port]>/api/oidc/callback`，比如 `http://127.0.0.1:21114/api/oidc/callback`
...
使用 Swag 生成 API 文档，方便开发者理解和使用 API。
  ...
  2.PC端文档 `<your server[:port]>/swagger/index.html`
...
2.使用`docker compose`，参考 [WIKI](https://github.com/HanGYvv/rustdesk-api/wiki)

#### 下载release直接运行

[下载地址](https://github.com/HanGYvv/rustdesk-api/releases)
...
3.编译后台前端，前端代码在 [rustdesk-api-web](https://github.com/HanGYvv/rustdesk-api-web) 中
...
- [链接超时问题](https://github.com/HanGYvv/rustdesk-api/issues/92)
